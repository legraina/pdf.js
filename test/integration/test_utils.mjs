  ...
  return Promise.all(
    global.integrationSessions.map(async session => {
      ...
      await page.evaluateOnNewDocument(() => {
        // In this fork (ngx-extended-pdf-viewer) the built viewer only
        // bootstraps itself when this flag is set; normally Angular calls
        // webViewerLoad() instead.
        globalThis.STANDALONE_VIEWER = true;
        // The fork also no longer assigns window.PDFViewerApplication
        // (viewer.js exposes it via this event for Angular instead), but
        // the tests drive the viewer through that global.
        // The editor toolbar buttons are bound by toolbar.js itself when
        // STANDALONE_VIEWER is set, so no extra shim is needed for them.
        document.addEventListener("ngxViewerFileHasBeenLoaded", evt => {
          window.PDFViewerApplication = evt.detail.PDFViewerApplication;
        });
